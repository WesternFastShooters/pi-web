...
test("custom extension UI forwards a component's complete structured transcript", async () => {
  const source = await readFile(new URL("./rpc-manager.ts", import.meta.url), "utf8");
  const renderSource = source.slice(
    source.indexOf("private emitCustomUiRender"),
    source.indexOf("private closeCustomUi"),
  );

  assert.match(renderSource, /getTranscriptEntries/);
  assert.match(renderSource, /transcriptEntries/);
});
